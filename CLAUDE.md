...
- **`Error::Api` with the `NO_HTTP_STATUS` sentinel** (`0`, in `error.rs`) — the HTTP call succeeded, the *operation* inside it failed. A `--wait` on a CICD op whose progress ends `"3"` (failed) or `"4"` (cancelled) is the case. A sentinel rather than `Option<u16>` because every other `Error::Api` site has a real status and would become `Some(..)` noise; `to_stderr_json` drops the key and `Display` drops the `" (404)"` suffix.
...
- **`count2` emits a delta, not a total** — `{"count": "+1"}` / `{"count": "-1"}`, as strings. Seed from `sn aggregate <TABLE> --count --query <ENCODED_QUERY>` (same query as the watch) and accumulate.
...
- **Every destructive command is gated by `cli::table::confirm_destructive(yes, verb, what)`** (`confirm_delete` is the `delete`-verb shim `attachment`/`cmdb` still call). On a TTY it prompts `<Verb> <what>? [y/N]:` on stderr; with a non-terminal stdin it refuses with ``"<verb> <target> requires --yes when stdin is not a terminal"``.
...
- Shared `pub(crate)` helpers live in `cli/table.rs`: `build_profile`, `build_client`, `build_client_with_headers`, `bool_opt`, `take_field`, `unwrap_or_raw`, `write_response`, `confirm_destructive`/`confirm_delete`. `format_from_flags` is **private** on purpose — it plus `emit_value` was the bypass that let `aggregate`, `scores list`, `scores favorite` and `open` accept `--output table` and ignore it, so the bypass is now a compile error rather than a convention.
- Every command's final emission goes through `cli::table::write_response(global, &value)`, which routes to `output::write_value` (JSON, default/raw) or `output_table::write_table` (`--output table`). `unwrap_or_raw` treats `Table` like `Default` (envelope unwrapped, then rendered).
