...
- **`Error::Api` with the `NO_HTTP_STATUS` sentinel** (`0`, in `error.rs`) — the HTTP call succeeded, the *operation* inside it failed. Two producers: a `--wait` on a CICD op whose progress ends `"3"` (failed) or `"4"` (cancelled), and an AMB subscribe refusal (`amb.rs` — the only HTTP exchange there was the `101` upgrade, and the refusal is an in-band Bayeux reply). A sentinel rather than `Option<u16>` because every other `Error::Api` site has a real status and would become `Some(..)` noise; `to_stderr_json` drops the key and `Display` drops the `" (404)"` suffix.
- **`Error::Instance { message, detail }`** — an HTTP 200 whose *body* does not answer the question asked, so it must not be handed back as if it did. `sn user me`'s scripted fallback returning two rows (proof the `javascript:` term was silently dropped and the rows are strangers) is the case.
...
3. **The session dies under a healthy socket.** The instance reaps a watcher's HTTP session ~60–120s after it was minted (measured on a PDI; not idle expiry — the countdown showed ~89 min remaining — and neither AMB traffic, nor delivered events, nor `--hydrate`'s HTTP GETs prevent it, even though connect replies acknowledge the touches). The reaped session keeps answering `/meta/connect` with `successful: true` and keeps accepting re-arms; it just never delivers another event. The honest signal is the same `ext["glide.session.status"]`, present on **every connect reply**: `Amb::poll` raises a recoverable transport error the moment it stops saying `session.logged.in`, and the supervisor reconnects — its `connect` closure re-mints cookies per attempt, so the replacement session is fresh. A long watch therefore reconnects every minute or two *by design*, one gap marker per reap; before 0.13.0 the field was checked only at handshake, and a watch went permanently, silently dead at the first reap while exiting 0.
...
- **`--display-value` defaults to `true`** on `table`, `change`, `aggregate`, `scores` (not `watch`, where it only narrows a `--hydrate` fetch). This also localizes dates into the caller's timezone/locale, and such a date will not round-trip into an encoded query — use `--display-value false` when a value has to be fed back to the API.
...
- **The read verb is optional on `table` and `cmdb`** (`IMPLIED_VERB_GROUPS`): `sn table incident <sys_id>` → `get`, `sn table incident` → `list`. Implemented in `cli::parse` as a *retry after failure*, not `allow_external_subcommands` — argv is re-parsed with a verb spliced in only when the first parse raised `InvalidSubcommand`. Two properties carry it, and both must survive any change here:
  - **The verb is decided, not guessed.** `get` requires a second positional and `list` rejects one, so at most one of the two rewrites can parse. Never infer a write.
