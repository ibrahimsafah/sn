...
- **Rotation outruns the reaper.** ServiceNow reaps a watcher's HTTP session every minute or two; the watcher preemptively replaces its session every 45s (`--session-rotate <SECS>`, `0` disables) with an overlapped, deduplicated handoff — no gap, no marker. A marker means a genuine failure (network drop, or rotation lost its race to the reaper); when completeness matters reconcile from shortly before the marker's window, since detection of a real death can lag one ~30s poll.
